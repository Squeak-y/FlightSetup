...
if getattr(sys, 'frozen', False):
    ...
    # If the application is run as a script (python .\script_name.py)
...
def is_font_present(font_name):
    root = tk.Tk()  # Create a dummy tkinter root window
    available_fonts = list(tkFont.families())
    root.destroy()  # Close the dummy window
...
def get_path_from_config(resource_name, default=None):
    ...
    # Build the path to the config.ini based on the current directory
    config_path = os.path.join(current_directory , 'config.ini')

    config = configparser.ConfigParser()
    config.read(config_path)
    
    # If 'Paths' is a section in the config
    if 'Paths' not in config.sections():
        raise Exception("The 'Paths' section is not found in config.ini!")
    
    # If resource_name exists in 'Paths' section
    if resource_name not in config['Paths']:
        if default:
            return default
        raise Exception(f"'{resource_name}' not found in 'Paths' section of config.ini!")
    
    # Here's the change: join the directory path with the filename/resource name
